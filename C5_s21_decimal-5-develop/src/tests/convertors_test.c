...
START_TEST(s21_from_decimal_to_int_er) {
  int dest = 0;
  s21_decimal var = {{UINT_MAX, 0, 0, 0}};
  int ret2 = s21_from_decimal_to_int(var, &dest);
  ck_assert_int_eq(ret2, 1);
}
END_TEST

START_TEST(s21_from_decimal_to_int_with_exp) {
  int dest = 0;
  s21_decimal var = {{105, 0, 0, 0}};
  s21_add_exp_decimal(&var, 1);
  int ret2 = s21_from_decimal_to_int(var, &dest);
  ck_assert_int_eq(ret2, 0);
  ck_assert_int_eq(10, dest);
}
END_TEST
...
START_TEST(s21_from_float_to_decimal_test_inf) {
  double src = INFINITY;
  s21_decimal dest = {0};
  int ret = s21_from_float_to_decimal(src, &dest);
...
Suite *convertors_suite(void) {
  ...
  tc = tcase_create("decimal to int");

  // ---------- Convertors and parsers ----------
  tcase_add_test(tc, s21_from_decimal_to_int_pos);
  tcase_add_test(tc, s21_from_decimal_to_int_neg);
  tcase_add_test(tc, s21_from_decimal_to_int_er);
  tcase_add_test(tc, s21_from_decimal_to_int_with_exp);
  ...
  tcase_add_test(tc, s21_from_float_to_decimal_test_inf);
