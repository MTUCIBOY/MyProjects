...
TEST(test_class, rc_constructor) {
  S21Matrix m(69, 420);
  EXPECT_EQ(m.Getrow(), 69);
  EXPECT_EQ(m.Getcol(), 420);
}
...
TEST(test_setters, Setrow_zero) {
  S21Matrix m;

  EXPECT_ANY_THROW(m.Setrow(0));
}

TEST(test_setters, Setcol_up) {
  S21Matrix m(2, 2);
  m(1, 1) = 6.9;
...
TEST(test_setters, Setcol_zero) {
  S21Matrix m;

  EXPECT_ANY_THROW(m.Setcol(0));
}
